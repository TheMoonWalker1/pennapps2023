...
loaded_xgb_model = load_xgboost_model('backend/xgb_model.pkl')
...
def get_data(longitude, latitude):
    ...
    print("Data at latitude {} and longitude {} on {}: {}".format(latitude, longitude, timestamp, i))
    ...
    ).get(band['id']).getInfo() for band in i['bands']}
    ...
    user_input['lat'] = float(latitude)
    user_input['lon'] = float(longitude)
